...
public class PortalCameraXR : MonoBehaviour
{
    [SerializeField]
    ...
    void UpdateCamera(ScriptableRenderContext SRC, Camera camera)
    {
        ...
        {
            RenderPortal(portals[0], portals[1], tempTexture1, SRC);
        }

        if (portals[1].Renderer.isVisible)
        {
            RenderPortal(portals[1], portals[0], tempTexture2, SRC);
        }
    }

    private void RenderPortal(Portal inPortal, Portal outPortal, RenderTexture targetTexture, ScriptableRenderContext SRC)
    {
        portalCamera.targetTexture = targetTexture;

        for (int i = iterations - 1; i >= 0; --i)
        {
            RenderCamera(inPortal, outPortal, i, SRC);
        }
    }
